...
public class RejectTalkCommandHandlerTests
{
    [Fact]
    public async Task Handle_ValidCommand_RejectsTalk()
    {
        // Arrange
        ...
        var handler = new RejectTalkCommandHandler(repository, currentUserService);
        var talkId = conference.Talks[0].Id.Value;
        var command = new RejectTalkCommand(conference.Id.Value, talkId);
        ...
        await repository
            .Received(1)
            .Save(Arg.Is<ConferenceAggregate>(c => c.Talks[0].Status == TalkStatus.Rejected));
    }
    ...
    public async Task Handle_ConferenceNotFound_ThrowsInvalidOperationException()
    {
        ...
        var handler = new RejectTalkCommandHandler(repository, currentUserService);
        var command = new RejectTalkCommand(ConferenceGuidV7.NewGuid(), ConferenceGuidV7.NewGuid());
    ...
    public async Task Handle_InvalidConferenceGuidV7_ThrowsArgumentException()
    {
        ...
        var command = new RejectTalkCommand(invalidGuid, ConferenceGuidV7.NewGuid());
