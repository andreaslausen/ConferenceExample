...
public class AcceptTalkCommandHandlerTests
{
    [Fact]
    public async Task Handle_ValidCommand_AcceptsTalk()
    {
        // Arrange
        ...
        var handler = new AcceptTalkCommandHandler(repository, currentUserService);
        var talkId = conference.Talks[0].Id.Value;
        var command = new AcceptTalkCommand(conference.Id.Value, talkId);
        ...
        await repository
            .Received(1)
            .Save(Arg.Is<ConferenceAggregate>(c => c.Talks[0].Status == TalkStatus.Accepted));
    }
    ...
    public async Task Handle_ConferenceNotFound_ThrowsInvalidOperationException()
    {
        ...
        var handler = new AcceptTalkCommandHandler(repository, currentUserService);
        var command = new AcceptTalkCommand(ConferenceGuidV7.NewGuid(), ConferenceGuidV7.NewGuid());
    ...
    public async Task Handle_InvalidConferenceGuidV7_ThrowsArgumentException()
    {
        ...
        var command = new AcceptTalkCommand(invalidGuid, ConferenceGuidV7.NewGuid());
