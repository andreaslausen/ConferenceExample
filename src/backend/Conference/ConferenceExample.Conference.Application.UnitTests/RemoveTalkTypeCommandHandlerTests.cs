...
public class RemoveTalkTypeCommandHandlerTests
{
    [Fact]
    public async Task Handle_ValidCommand_RemovesTalkType()
    {
        // Arrange
        var repository = Substitute.For<IConferenceRepository>();
        var conference = CreateConferenceWithTalkType();
        var currentUserService = CreateMockCurrentUserService(conference.OrganizerId);
        var handler = new RemoveTalkTypeCommandHandler(repository, currentUserService);
        var talkTypeId = conference.TalkTypes[0].Id.Value;
        var command = new RemoveTalkTypeCommand(conference.Id.Value, talkTypeId);
        ...
        await repository.Received(1).Save(Arg.Is<ConferenceAggregate>(c => c.TalkTypes.Count == 0));
    }
    ...
    public async Task Handle_ConferenceNotFound_ThrowsInvalidOperationException()
    {
        ...
        var handler = new RemoveTalkTypeCommandHandler(repository, currentUserService);
        var command = new RemoveTalkTypeCommand(
    ...
    public async Task Handle_InvalidConferenceGuidV7_ThrowsArgumentException()
    {
        ...
        var command = new RemoveTalkTypeCommand(invalidGuid, ConferenceGuidV7.NewGuid());
    ...
    private static ConferenceAggregate CreateConferenceWithTalkType()
    {
