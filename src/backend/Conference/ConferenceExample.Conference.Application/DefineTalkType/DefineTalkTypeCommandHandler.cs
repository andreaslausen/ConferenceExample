    ...
    public async Task<TalkTypeDefinedDto> Handle(DefineTalkTypeCommand command)
    {
        ...
        if (conference.OrganizerId.Value != currentOrganizerId.Value)
        {
            throw new UnauthorizedAccessException(
                $"User {currentUserId} is not authorized to define talk types for conference {conference.Id.Value}. Only the organizer who created the conference can define talk types."
            );
        }
