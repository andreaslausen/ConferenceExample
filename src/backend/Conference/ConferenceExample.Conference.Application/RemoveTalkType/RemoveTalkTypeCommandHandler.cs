    ...
    public async Task Handle(RemoveTalkTypeCommand command)
    {
        ...
        if (conference.OrganizerId.Value != currentOrganizerId.Value)
        {
            throw new UnauthorizedAccessException(
                $"User {currentUserId} is not authorized to remove talk types for conference {conference.Id.Value}. Only the organizer who created the conference can remove talk types."
            );
        }
