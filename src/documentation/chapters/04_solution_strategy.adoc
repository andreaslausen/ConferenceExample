...
* *Event Store per module* — each module persists its aggregate state as an immutable sequence of domain events via `IEventStore`. No module may access another module's event stream directly. The concrete persistence technology behind `IEventStore` is not yet decided (see open decision OD-3); the current implementation uses an in-memory event store as a placeholder.
...
* *Persistence* — contains the repository implementations for the bounded context. Repositories load and save aggregate state as domain events by using the `IEventStore` interface provided by the shared infrastructure. Depends on the domain.
